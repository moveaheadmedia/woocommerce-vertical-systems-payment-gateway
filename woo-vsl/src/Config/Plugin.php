...
use WooVsl\Common\Traits\Singleton;
...
final class Plugin {
	...
	public function data(): array {
		$plugin_data = apply_filters( 'woo_vsl_plugin_data', [
			'settings'               => get_option( 'woo-vsl-settings' ),
			'plugin_path'            => untrailingslashit(
				plugin_dir_path( WOO_VSL_PLUGIN_FILE )  // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
			),
			'plugin_template_folder' => 'templates',
			'ext_template_folder'    => 'woo-vsl-templates',
			/**
			 * Add extra data here
			 */
		] );
		return array_merge(
			apply_filters( 'woo_vsl_plugin_meta_data',
				get_file_data( WOO_VSL_PLUGIN_FILE, // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
					[
						'name'         => 'Plugin Name',
						'uri'          => 'Plugin URI',
