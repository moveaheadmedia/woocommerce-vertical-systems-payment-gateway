...
use WooVsl\Common\Traits\Requester;
use WooVsl\Common\Utils\Errors;
use WooVsl\Config\Classes;
use WooVsl\Config\I18n;
use WooVsl\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'woo_vsl_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'woo_vsl_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'woo-vsl' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'Woo VSL classes are not being loaded by Composer\'s Autoloader' ),
